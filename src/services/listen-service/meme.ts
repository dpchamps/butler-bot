...
} : speak(`I couldn't make that meme. The reason is quite technical, you see`);
...
const searchForMeme = async (memeName: Option<string>) => {
    if(!memeName){
        throw new Error(`I expected this command: \`butler: meme. <meme name>. <top text>. <bottom text>\``);
    }
    const regex = new RegExp(memeName, "gi");
    const memes = orDefault(
        memeCache.data,
        (await fetch("https://api.imgflip.com/get_memes").then(res => res.json()).then(ImgFlipGetResponse.check)).data.memes
    );
...
export const meme = (message: Message, {content}: BotCommand, config: AppConfig) => {
    ...
    return searchForMeme(memeName)
        ...
        .then(
        templateId => makeMeme(memeGenerateFormData({
