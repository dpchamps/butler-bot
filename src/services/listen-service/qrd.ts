...
import {Option, orDefault, parseToNumber, tail, unimplemented} from "../../util";
...
const searchError = () => new Error(`I had a hard time finding messages in the channel.`);


const validateValue = (value: number) => {
    if(value < 10 || value > 200){
        throw new Error(`Can only run the qrd between 10 and 200 messages. One would be _wise_ to follow the chat more closely.`);
    }
...
const fetchMessagesUntilSatisfied = async (needle: Date, start: Message, messages?: Collection<string, Message>): Promise<Message[]> => {
    const lastMessages = (await start.channel.messages.fetch({
        limit: 10,
        before: start.id
    }))
        .sort((a, b) => compareDsc(new Date(a.createdTimestamp), new Date(b.createdTimestamp)))
...
const summarizeMessage = async (messages: Message[]) => {
    const corpus = messages.map(msg => `${msg.content.split("\n").filter(Boolean).join(".")}.`).join("\n");

    const summarizer = new SummarizerManager(corpus, 5);

    const trSummary = await summarizer.getSummaryByRank().then(({summary}: any) => summary);

    return (trSummary as string).split("\n").filter(Boolean).join("\n");
};

const fetchMessages = async (message: Message|undefined, remaining: number): Promise<Message[]> => {
    if(!message) return [];
...
export const qrd = async (message: Message, {content}: BotCommand) => {
    try{
        ...
        await message.channel.send({
            content: speak(`Your qrd, as requested of the last ${limit} messages:\n${summary}`),
