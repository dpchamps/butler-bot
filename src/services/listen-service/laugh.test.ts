...
describe("laugh", () => {
    ...
    it("Should laugh at a user with a funny image", async () => {
        ...
        expect(promisesMock.readdir).toHaveBeenCalledWith(expect.stringMatching("girls-laughing"));
    ...
    it("Should apologize if no reference is found", async () => {
        ...
        expect(fakeMessage.channel.send).toHaveBeenCalledWith(
            expect.stringMatching("I'm not quite sure whom it is you'd like me to laugh at.")
        );
    });
